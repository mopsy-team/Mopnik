 ...
 * @author Daniel Stahr
 */
public class MopPointPainter extends WaypointPainter<MopPoint> {


    @Override
    protected void doPaint(Graphics2D g, JXMapViewer jxMapViewer, int width, int height) {
        for (MopPoint mopPoint : getWaypoints()) {
            Point2D point = jxMapViewer.getTileFactory().geoToPixel(
                    mopPoint.getPosition(), jxMapViewer.getZoom());
            ...
            JButton button = mopPoint.getButton();
